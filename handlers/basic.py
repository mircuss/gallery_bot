...
async def photo(message: Message, tag: TagRepo):
    ...
    msg = await message.send_copy(chat_id=settings.channel_id)
...
async def tag(call: CallbackQuery, tag: TagRepo, tag_message: MessageTagRepo):
    ...
    tag = await tag_message.get(message_id=call.message.message_id,
                                tag_id=tag_id)
    if tag:
        await tag_message.delete(message_id=message_id,
                                 tag_id=tag_id)
        msg = await call.message.answer(text=f" REMOVE {tag_name.name}")
        await asyncio.sleep(10)
        await msg.delete()
        return
    await tag_message.create(message_id=message_id, tag_id=tag_id)
...
async def next(call: CallbackQuery, tag: TagRepo):
    ...
    if last >= len(tags):
        return
    markup = generate_tag_keyboard(tags=tags[last:last+10],
                                   last=last+10,
